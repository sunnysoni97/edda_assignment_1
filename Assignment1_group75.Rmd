...
(a) 
To perfrom a three-way ANOVA test, we need to check normality. This is done by using a QQ-plot and the Shapiro-Wilk test. The line in the QQ-plot looks linear and the Shapiro-Wilk test had a p-vaue larger than 0.05 which means that the data comes from normal distribution. Therefore, a three-way ANOVA can be performed. 
```{r, echo=FALSE, results='hide'}
data = read.table(file="cream.txt",header=TRUE)
df = data.frame(data)
head(df)
df = numeric(df)
df = sample_n_by(df, batch, position, starter, size = 1)
df = group_by(df, batch, position, starter)
model  <- lm(acidity ~ batch*position*starter, data = df)
ggqqplot(residuals(model))
shapiro_test(residuals(model))
```
```{r, echo=FALSE, results='hide'}
anova_5 = aov(acidity ~ batch+position+starter+batch:position+batch:starter+position:starter+ batch:position:starter, data=df)
summary(anova_5)
```
There are no main effects between different variables. 
