...
The CI-interval for this t-test is [-∞, 14.6], which means that we can be 95% sure that the mean of the data sample lies within the CI-interval.
...
for(i in 1:B) {
  ...
  pttest[i]=t.test(waiting_t, mu =15, alternative="less")[[3]] ## extract p-value
  psign[i]=binom.test(above_m, length(waiting_t), p=0.5, alternative = "less")[[3]]} 
sum(psign<0.05)/B
sum(pttest<0.05)/B
...
for(i in 1:B) {
  ...
  pttest2[i]=t.test(waiting_t, mu =15, alternative="less")[[3]] ## extract p-value
  psign2[i]=binom.test(above_m, length(waiting_t), p=0.5, alternative = "less")[[3]]}
sum(psign2<0.05)/B
sum(pttest2<0.05)/B
...
n = 15 #sample size
SE= (sd(waiting_t))/sqrt(n) #Calculating standard error by 
SE
...
summary(model_h)
```
From the two-way ANOVA test, we can see that the treatment variable 'rate' has a significant effect on hemoglobin, since the p-value is below 0.05. However, the block variable 'method' does not have a significant effect on hemoglobin since it's p-value is 0.216, which is above 0.05. 
The p-value for rate:method is 0.377, which means that there is no evidence for interaction. 
