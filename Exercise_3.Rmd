...
```
This shows that the p-value is <0.05, which means that it is statistically significant. 
And it is worth mentioning that it only takes one factor into account which is the activity and not the thorax! 
Note: the outcome is not fully reliable because the ANOVA is supposed to be performed on data that is normally distributed which it is not. 
Because you create a linear model which creates data that is normal therefore ANOVA can be used! 
...
It could therefore suggest that when there is more sexual activity of a fruitflies the (log)longevity decreases with it. 
------------------------------------------------------------
...
```
In order to know whether thorax is an interaction term model2 has been created. The thorax parameter has a pvalue < 0.05 which is 3.151e-13. 
Conclusion: This suggests that the thorax does indeed influence the loglongevity of the fruitflies. 
...
The analysis with the thorax length should not be included because there should not be interaction between the sexual activity and the thorax length. This is because the experimenters randomly chose the sexual activity that the flies were going to have and not based on thorax length. 
Besides, when thorax length was included there was no significant difference found. 
...
```
Changing the loglongevity to longevity does not change the statistical significance of a parameter. For example for longevity the thorax is significant which is also for loglongevity. For the interaction between thorax and activity the parameter is not significant for both loglongevity and longevity. 
However it is noteworty mentioning that logarithmic transformation is a convenient means of transforming a highly skewed variable into a more normalized dataset. When modeling variables with non-linear relationships, the chances of producing errors may also be skewed negatively.
Conclusion: 
Given that the data is not normal the logarithm can be used for transformation of skewed datasets into a normalized dataset. Which explains the usage of logarithms, however it does not make a difference in terms for pvalues. Therefore the usage without logarithm transformation is wrong. 
